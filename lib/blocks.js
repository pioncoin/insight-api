...
BlockController.prototype.checkBlockHash = function(req, res, next) {
  ...
  if (hash.length < 64 || !isHexadecimal(hash)) {
    this.node.services.piond.getBlockHeader(parseInt(hash), function(err, info) {
      if (err) {
...
BlockController.prototype.blockHeader = function(req, res, next) {
	...
	self.node.services.piond.getBlockHeader(hash, function(err, info) {
		if (err) {
			return self.common.handleErrors(err, res);
		}
...
BlockController.prototype.blockHeaders = function(req, res, next) {
	...
	if(blockIdentifier.length===64){
		...
		var result = self.node.services.piond.getBlockHeaders(hash, cb, nbOfBlockToFetch);
	}else{
		var height = blockIdentifier;
		var result = self.node.services.piond.getBlockHeaders(height, cb, nbOfBlockToFetch);
	}
...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    blockCached.confirmations = self.node.services.piond.height - blockCached.height + 1;
...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix pioncore to give back null instead of null hash
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  this.node.services.piond.getBlockHeader(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.piond.getRawBlock(hash, function(err, blockBuffer) {
      if (err) {
        return next(err);
      }

      var br = new pioncore.encoding.BufferReader(blockBuffer);
      ...
      // would require changes in pioncore-node.
      var header = pioncore.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [pioncore.Transaction().fromBufferReader(br)];

      self.node.services.piond.getBlockHeader(hash, function(err, blockHeader) {
        if (err) {
          return next(err);
        }
        ...
        var confirmations = self.node.services.piond.height - height + 1;
...
BlockController.prototype.list = function(req, res) {
  ...
  self.node.services.piond.getBlockHashesByTimestamp(lte, gte, function(err, hashes) {
    if(err) {
      return self.common.handleErrors(err, res);
    }
...
BlockController.prototype.getPreviousBlock = function(prevHash, cb) {
  ...
  if(prevHash===null) cb(null,null);
      ...
      self.node.getBlock(prevHash, function (err, block) {
          ...
          self.node.services.piond.getBlockHeader(prevHash, function (err, info) {
              if (err) {
...
BlockController.prototype.getBlockReward = function(prevHash, cb) {
  ...
  self.getPreviousBlock(prevHash, function(err, info) {
    ...
    if(info===null){
      ...
      var reductionFactor = 0.84096;
      var COIN = 100000000;

      if (nPrevHeight == 0) { 
        nSubsidyBase = 1000000; // Dev
    } 
    else if (nPrevHeight == 1) { 
        nSubsidyBase =  250000; // Marketing
    }
    else {
        nSubsidyBase = 20;
    }
    var nSubsidy = nSubsidyBase * COIN;


    for (var i = nSubsidyHalvingInterval; i <= nPrevHeight; i += nSubsidyHalvingInterval) {
      nSubsidy = nSubsidy * reductionFactor;
    }

     // 100 Bonus Coin every 2190 blocks for the first 5 years 
    if (nPrevHeight < 1314000) { // 5 years
      if ((nPrevHeight + 1) % 2190 == 0) {
        nSubsidy = nSubsidy + (100 * COIN); // 100 PION every 2190 blocks
      }
	    cb(null, parseFloat(nSubsidy.toString(10)).toFixed(8));
    }
  }
  });
};

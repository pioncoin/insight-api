<h1 align="center">Insight-api-pion</h1>

<div align="center">
  <strong>A Pion blockchain REST and WebSocket API Service</strong>
</div>
<br />
<div align="center">
  <!-- Stability -->
  ...
  <!-- Build Status -->
  <a href="https://travis-ci.org/pioncoin/insight-api-pion">
    <img src="https://img.shields.io/travis/pioncoin/insight-api-pion/master.svg?style=flat-square" alt="Build Status" />
  </a>
  <!-- NPM version -->
  <a href="https://npmjs.org/package/insight-api-pion">
    <img src="https://img.shields.io/npm/v/insight-api-pion.svg?style=flat-square" alt="NPM version" />
  </a>
</div>
...
```
With all the configuration options available: https://github.com/pioncoin/insight-api-pion/blob/master/lib/ratelimiter.js#L10-17
...
### Block
```
  /insight-api-pion/block/[:hash]
  /insight-api-pion/block/0000000006e7b38e8ab2d351239019c01de9a148b5baef58cfe52dfd9917cedc
```

### Block Index
Get block hash by height
```
  /insight-api-pion/block-index/[:height]
  /insight-api-pion/block-index/0
```
This would return:
```
{
...
### Raw Block
```
  /insight-api-pion/rawblock/[:blockHash]
```
...
```
  /insight-api-pion/blocks?limit=3&blockDate=2017-04-22
```
...
### Transaction
```
  /insight-api-pion/tx/[:txid]
  /insight-api-pion/tx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
  /insight-api-pion/rawtx/[:rawid]
  /insight-api-pion/rawtx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
```
...
GET method:
```
  /insight-api-pion/gobject/deserialize/[:hexData]
  /insight-api-pion/gobject/deserialize/5b5b2270726f706f736[..]
```
...
GET method:
```
  /insight-api-pion/gobject/votes/current/[:hash]
  /insight-api-pion/gobject/votes/current/fbda8cdc1f48917f53b7d63fbce81c85d6dedd3d0e476e979926dfd154b84034
```
...
GET method:
```
  /insight-api-pion/governance/budget/[:blockIndex]
  /insight-api-pion/governance/budget/79872
```
...
POST method:
```
  /insight-api-pion/gobject/submit
```
...
### Masternodes List
```
  /insight-api-pion/masternodes/list
```
### Validate Masternode
```
...
```
  /insight-api-pion/sync
```

### Live Network P2P Data Sync Status
```
  /insight-api-pion/peer
```

### Status of the Bitcoin Network
```
  /insight-api-pion/status?q=xxx
```
...
- The response for the `/sync` endpoint does not include `startTs` and `endTs` as the sync is no longer relevant as indexes are built in piond.
- The endpoint for `/peer` is no longer relevant connection to piond is via ZMQ.
...
The `/status?q=getTxOutSetInfo` method has also been removed due to the query being very slow and locking piond.
...
- (Medium)[How to setup a Pion Instant-Send Transaction using Insight API?????????The comprehensive way](https://medium.com/@obusco/setup-instant-send-transaction-the-comprehensive-way-a80a8a0572e)
